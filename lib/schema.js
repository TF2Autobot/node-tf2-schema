    ...
    getItemObjectFromName(name) {
        ...
        for (const wear in wears) {
            if (name.includes(wear)) {
                log('Wear - before (L568)', { name, item });
                name = name.replace(wear, '').trim();
                item.wear = wears[wear];
                log('Wear - after (L571)', { name, item });
                break;
            }
        }
        ...
        if (name.includes('strange(e)')) {
            log('"Strange(e)" - before (L579)', { name, item });
            item.quality2 = 11;
            isExplicitElevatedStrange = true;
            name = name.replace('strange(e)', '').trim();
            log('"Strange(e)" - after (L583)', { name, item });
        }

        // Keep this in case it is an elevated strange
        // such Strange Unique/Genuine/Haunted/Unusual, etc.
        if (name.includes('strange')) {
            log('"Strange" - before (L589)', { name, item });
            item.quality = 11;
            name = name.replace('strange', '').trim();
            log('"Strange" - after (L592)', { name, item });
        }

        if (name.includes('non-craftable')) {
            log('"Non-Craftable" - before (L597)', { name, item });
            name = name.replace('non-craftable', '').trim();
            item.craftable = false;
            log('"Non-Craftable" - after (L600)', { name, item });
        }
        ...
        if (name.includes('non-tradable')) {
            log('"Non-Tradable" - before (L608)', { name, item });
            name = name.replace('non-tradable', '').trim();
            item.tradable = false;
            log('"Non-Tradable" - after (L611)', { name, item });
        }
        ...
        for (const effectC in this.effects) {
            ...
            if (name.includes(effect)) {
                log('"Effect" - before (L794)', { name, item });
                ...
                log('"Effect" - after (L807)', { name, item });
                break;
            }
        }
        ...
        if (item.wear) {
            for (const paintkitC in this.paintkits) {
                ...
                if (name.includes(paintkit)) {
                    log('"Paintkit" - before (L828)', { name, item });
                    ...
                    log('"Paintkit" - after (L849)', { name, item });
                    break;
                }
            }
        ...
        if (name.includes('(paint: ')) {
            log('"Painted" - before loop (L896)', { name, item });
            ...
            for (const paintC in this.paints) {
                ...
                if (name.includes(paint)) {
                    log('"Painted" - in loop, before (L903)', { name, item });
                    name = name.replace(paint, '').trim();
                    item.paint = this.paints[paintC];
                    log('"Painted" - after (L906)', { name, item });
                    break;
                }
            }
        }
        ...
        if (name.includes('kit fabricator') && item.killstreak > 1) {
            log('"Kit Fabricator" - before (L913)', { name, item });
            ...
            log('"Kit Fabricator" - after (L936)', { name, item });
        }
        ...
        if (
            ...
            log('"Collector\'s Chemistry Set" - before (L943)', { name, item });
            ...
            log('"Collector\'s Chemistry Set" - after (L962)', { name, item });
        }

        if (name.includes('strangifier chemistry set')) {
            log('"Strangifier Chemistry Set" - before (L966)', { name, item });

            name = name.replace('strangifier chemistry set', '').trim();
            const schemaItem = this.getItemByItemName(name);
            if (!schemaItem) {
                log('returned L971', { name, item });
                return item;
            }
            ...
            log('"Strangifier Chemistry Set" - after (L982)', { name, item });
        }

        if (name.includes('strangifier')) {
            log('"Strangifier" - before (L986)', { name, item });
            ...
            const schemaItem = this.getItemByItemName(name);
            if (!schemaItem) {
                log('returned L993', { name, item });
                return item;
            }

            item.target = schemaItem.defindex;
            item.quality = item.quality ?? schemaItem.item_quality; //default quality
            log('"Strangifier" - after (L999)', { name, item });
        }

        if (name.includes('kit') && item.killstreak) {
            log('"Kit" - before (L1003)', { name, item });
            ...
            log('"Kit" - after (L1029)', { name, item });
        }

        if (item.defindex) {
            log('returned L1031', { name, item });
            return item;
        }

        if (typeof item.paintkit === 'number' && name.includes('war paint')) {
            log('"War Paint" - before (L1036)', { name, item });
            ...
            if (name.includes('salvaged mann co. supply crate #')) {
                ...
                log('"Mann Co. Supply Crate" (L1069)', { name, item });
            ...
            if (schemaItem.item_class === 'supply_crate') {
                log('with "supply_crate" - before (L1132)', { name, item });
                item.crateseries = this.crateSeriesList[item.defindex];
                log('with "supply_crate" - after (L1134)', { name, item });
            } else if (schemaItem.item_class !== 'supply_crate' && number !== null) {
                log('not "supply_crate" and number !== null - before (L1136)', { name, item });
                item.craftnumber = number;
                log('not "supply_crate" and number !== null - after (L1138)', { name, item });
            }
        }

        log('Final returned L1142', { name, item });
        return item;
    }
