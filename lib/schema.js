...
class Schema {
    ...
    getItemByItemNameWithThe(name) {
        ...
        for (let i = 0; i < itemsCount; i++) {
            const item = items[i];
            let itemName = removeAccents(item.item_name.toLowerCase());
    ...
    getItemByItemName(name) {
        ...
        for (let i = 0; i < itemsCount; i++) {
            const item = items[i];
            if (removeAccents(name.toLowerCase()) === removeAccents(item.item_name.toLowerCase())) {
                if (item.item_name === 'Name Tag' && item.defindex === 2093) {
                    // skip and let it find Name Tag with defindex 5020
                    continue;
                }
...
function removeAccents(str) {
    // Source: Google Gemini (https://share.google/aimode/D4jdOfLCSayfBkwVM)
    // Affected names: L'Étranger, Détective Noir, Quäckenbirdt, Jöten, etc.
