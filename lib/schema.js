...
const retiredKeys = {
    ...
    5791: { defindex: 5791, name: 'Naughty Winter Crate Key 2014' }, // Non-Craftable
    5792: { defindex: 5792, name: 'Nice Winter Crate Key 2014' } // Non-Craftable
};
...
class Schema {
    ...
    getItemObjectFromName(name) {
        ...
        for (const effect in effects) {
            ...
            if (
                effect === 'hot' &&
                !name.includes('hot ') &&
                (name.includes('shotgun') || name.includes('plaid potshotter') || name.includes('shot to hell'))
            ) {
                continue;
            }
    ...
    checkExistence(item) {
        ...
        if (retiredKeys[String(item.defindex)] !== undefined) {
            if ([5713, 5716, 5717, 5762, 5791, 5792].includes(item.defindex)) {
