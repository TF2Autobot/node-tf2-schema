...
class Schema {
    ...
    _extractItemsGameLookups() {
        ...
        this.festivizableDefindexes = new Set();
        this.strangifierTargets = new Map();
        ...
        for (const defindex in items) {
            ...
            if (item.tags && item.tags.can_be_festivized == 1) {
                this.festivizableDefindexes.add(Number(defindex));
            }
            ...
            if (target !== null && target !== undefined) {
                this.strangifierTargets.set(Number(defindex), Number(target));
            }
        }
    }
    ...
    _buildItemIndexes() {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (!byName.has(lower)) byName.set(lower, i);

            const normalized = normalizeNameKey(item.item_name);
            if (!byName.has(normalized)) byName.set(normalized, i);

            const noThe = removeThe(lower);
            if (!byNameNoThe.has(noThe)) byNameNoThe.set(noThe, i);

            const normalizedNoThe = removeThe(normalized);
            if (!byNameNoThe.has(normalizedNoThe)) byNameNoThe.set(normalizedNoThe, i);
        }
    ...
    getItemByItemNameWithThe(name) {
        ...
        const index = this._itemsByNameNoThe.get(n);
    ...
    getItemObjectFromName(name) {
        ...
        //Check for effects
        const excludeAtomic = ['bonk! atomic punch', 'atomic accolade', 'bonk! atomic kicks', 'atomic bomber'].some(ex => name.includes(ex));
    ...
    getItemByItemName(name) {
        const index = this._itemsByName.get(normalizeNameKey(name));
        return typeof index === 'number' ? this.raw.schema.items[String(index)] : null;
    }
