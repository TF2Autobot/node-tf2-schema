...
class Schema {
    ...
    getCrateSeriesList() {
        ...
        Loop1: for (let i = 0; i < itemsCount; i++) {
            if (items[i].attributes) {
                ...
                Loop2: for (let j = 0; j < attrCount; j++) {
                    if (attributes.name === 'set supply crate series') {
                        crateseries.push(attributes.value);
                        break Loop2;
                    }
                }
            }
        ...
        for (let i = 0; i < count; i++) {
            ...
            const seriesAttribute = items[defindex]?.static_attrs?.['set supply crate series'];
            if (seriesAttribute) {
                crateseries.push(Number(seriesAttribute?.value || seriesAttribute));
            }
        }

        items = undefined;

        const fixDuplicate = new Set(crateseries);
        return [...fixDuplicate].sort();
    }

    updateCrateSeriesList() {
        this.crateSeriesList = getCrateSeriesList();
    }
    ...
    checkExistance(item) {
        ...
        if (item.crateseries) {
            ...
            if (schemaItem.item_class !== 'supply_crate') {
                ...
                if (!this.crateSeriesList.includes(item.crateseries)) {
                    // if item.crateseries is not included in the crateSeriesList, does not exist.
                    return null;
                }

                // Check for specific crates/cases
                let foundCrateSeries = false;
                for (let i = 0; i < schemaItem.attributes?.length; i++) {
                    if (schemaItem.attributes[i].name === 'set supply crate series') {
                        foundCrateSeries = true;
                        if (item.crateseries !== schemaItem.attributes[i].value) {
                            return null;
                        }
                        break;
                    }
                ...
                if (!foundCrateSeries) {
                    const seriesAttribute =
                        this.raw.items_game.items[schemaItem.defindex]?.static_attrs['set supply crate series'];
                    const crs = Number(seriesAttribute?.value || seriesAttribute);
                    if (item.crateseries !== crs) {
                        return null;
                    }
                }
            } else if (
