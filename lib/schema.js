...
class Schema {
    ...
    getName(item, proper = true, usePipeForSkin = false, scmFormat = false) {
        ...
        if (scmFormat && item.wear !== null && item.item.effect !== null && item.quality === 15) {
...
function getAllSchemaItems(apiKey, next, items, callback) {
    ...
    requestRetry(
        {
            json: true,
            url: `http://api.steampowered.com/IEconItems_440/GetSchemaItems/v0001/?key=${apiKey}&start=${next}&language=${language}`,
            retryStrategy: (err, response) => {
                const statusCode = response ? response.statusCode : null;

                // 404 is sometimes returned, so we want to retry
                return (
                    statusCode && (statusCode === 429 || statusCode === 404 || (500 <= statusCode && statusCode < 600))
                );
            }
        },
        (err, response, body) => {
            ...
            if (!body) {
                // body not properly received, retry
                return getAllSchemaItems(apiKey, next, items, callback);
            }

            items = (items || []).concat(body.items);

            if (result.next !== undefined) {
                getAllSchemaItems(apiKey, body.next, items, callback);
            } else {
                callback(null, items);
            }
        }
