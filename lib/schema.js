...
class Schema {
     ...
     * @param {Boolean} [data.lite] Lite version with removed items_game
     */
    constructor(data) {
    ...
    setPropertiesData() {
        ...
        // Extracts crate series first (uses raw.items_game)
        this.crateSeriesList = this.getCrateSeriesList();

        // NEW: Always extract the lightweight lookups so helper methods work in both modes
        this._extractItemsGameLookups();

        // NEW: ONLY delete raw.items_game if lite mode is enabled
        if (this.lite && this.raw && this.raw.items_game) {
            delete this.raw.items_game;
        }
    ...
    /**
     * Extracts festivizable items and strangifier targets
     */
    _extractItemsGameLookups() {
        if (!this.raw || !this.raw.items_game || !this.raw.items_game.items) {
            return;
        }

        const festivizableDefindexes = new Set();
        const strangifierTargets = new Map();

        const items = this.raw.items_game.items;
        for (const defindex in items) {
            const item = items[defindex];

            // 1. Festivizable check
            if (item.tags && item.tags.can_be_festivized == 1) {
                festivizableDefindexes.add(Number(defindex));
            }

            // 2. Strangifier target check
            let target = null;
            if (item.attributes && item.attributes['tool target item']) {
                target = item.attributes['tool target item'].value;
            } else if (item.static_attrs && item.static_attrs['tool target item']) {
                target = item.static_attrs['tool target item'].value || item.static_attrs['tool target item'];
            }

            if (target !== null && target !== undefined) {
                strangifierTargets.set(Number(defindex), Number(target));
            }
        }

        this._festivizableDefindexes = festivizableDefindexes;
        this._strangifierTargets = strangifierTargets;
    }

    /**
     * Helper methods for tf2autobot
     */
    isFestivizable(defindex) {
        return this._festivizableDefindexes.has(Number(defindex));
    }

    getStrangifierTarget(defindex) {
        return this._strangifierTargets.get(Number(defindex)) ?? null;
    }
    ...
    _buildItemIndexes() {
        ...
        const itemsNameWithAtomic = new Set();
        const effectsSameNameAsPaintkit = new Set();

        const paintkits = new Set(Object.keys(this.paintkits).map(paintkit => paintkit.toLowerCase()));
        const effects = new Set(Object.keys(this.effects).map(effect => effect.toLowerCase()));
        const qualities = new Set(Object.keys(this.qualities).map(quality => quality.toLowerCase()));
        const itemsNameWithQuality = new Set();

        for (const quality of qualities) {
            for (const paintkit of paintkits) {
                // Example: Haunted Ghosts (236) for now, but might be more in the future.
                if (paintkit.includes(quality)) itemsNameWithQuality.add(paintkit);
            }
            for (const effect of effects) {
                if (effect === 'community sparkle') continue;
                // Example: Haunted Ghosts (8), Haunted Phantasm Jr(86), Haunted Phantasm (3011)
                // Haunted Kraken(257), Haunted Forever!(261),
                if (effect.includes(quality)) itemsNameWithQuality.add(effect);
            }
        }

        for (const effect of effects) {
            for (const paintkit of paintkits) {
                if (paintkit.includes(effect)) effectsSameNameAsPaintkit.add(effect);
            }
        }

        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            byDefindex.set(item.defindex, i);

            if (item.item_quality === 0) continue;
            if (item.item_name === 'Name Tag' && item.defindex === 2093) continue;

            const fullyNormalized = removeThe(normalizeNameKey(item.item_name));
            if (!byName.has(fullyNormalized)) byName.set(fullyNormalized, i);

            const itemName = normalizeNameKey(item.item_name).toLowerCase();
            if (itemName.includes('atomic')) itemsNameWithAtomic.add(itemName);
            if (
                ![
                    'unusualifier',
                    'strange part',
                ...
                ].some(itemEx => itemName.includes(itemEx))
            ) {
                for (const quality of qualities) {
                    // Example: Haunted Metal Scrap(267), Haunted Hat(30300), Vintage Tyrolean(101), Vintage Merryweather(182)
                    // And anything in the future that Valve might add, "Genuine Hat"? smh
                    if (itemName.includes(quality) && itemName.startsWith(quality)) itemsNameWithQuality.add(itemName);
                }
            }
        }

        this._effectsSameNameAsPaintkit = effectsSameNameAsPaintkit;
        this._itemsNameWithQuality = itemsNameWithQuality;
        this._itemsNameWithAtomic = itemsNameWithAtomic;
    ...
    getItemObjectFromName(name) {
        name = name.toLowerCase().replace('uncraftable', 'non-craftable').replace('noncraftable', 'non-craftable');
        ...
        for (const wear in wears) {
            if (name.includes(wear)) {
                log('Wear - before (L576)', { name, item });
                name = name.replace(wear, '').trim();
                item.wear = wears[wear];
                log('Wear - after (L579)', { name, item });
                break;
            }
        }
        ...
        if (name.includes('strange(e)')) {
            log('"Strange(e)" - before (L587)', { name, item });
            item.quality2 = 11;
            isExplicitElevatedStrange = true;
            name = name.replace('strange(e)', '').trim();
            log('"Strange(e)" - after (L591)', { name, item });
        }

        // Keep this in case it is an elevated strange
        // such Strange Unique/Genuine/Haunted/Unusual, etc.
        if (name.includes('strange')) {
            log('"Strange" - before (L597)', { name, item });
            item.quality = 11;
            name = name.replace('strange', '').trim();
            log('"Strange" - after (L600)', { name, item });
        }

        if (name.includes('non-craftable')) {
            log('"Non-Craftable" - before (L604)', { name, item });
            name = name.replace('non-craftable', '').trim();
            item.craftable = false;
            log('"Non-Craftable" - after (L607)', { name, item });
        }
        ...
        if (name.includes('non-tradable')) {
            log('"Non-Tradable" - before (L615)', { name, item });
            name = name.replace('non-tradable', '').trim();
            item.tradable = false;
            log('"Non-Tradable" - after (L618)', { name, item });
        }
        ...
        if (this._itemsNameWithQuality.has(qualitySearch) === false) {
        ...
        for (const effectC in this.effects) {
            ...
            if (this._effectsSameNameAsPaintkit.has(effect) && item.wear && effect !== 'hot') {
                // Haunted Ghosts, Pumpkin Patch, Stardust as of now, more probably in the future
                continue;
            }

            if (effect === 'atomic' && (name.includes('subatomic') || this._itemsNameWithAtomic.has(name))) {
                continue;
            }
            ...
            if (effect === 'frostbite' && (name.includes('frostbite bonnet') || name.includes('frostbite fit'))) {
                // Skip Frostbite effect if name include Frostbite Bonnet or Frostbite Fit
                continue;
            }
            ...
            if (name.includes(effect)) {
                log('"Effect" - before (L801)', { name, item });
                ...
                log('"Effect" - after (L814)', { name, item });
                break;
            }
        }
        ...
        if (item.wear) {
            for (const paintkitC in this.paintkits) {
                ...
                if (name.includes(paintkit)) {
                    log('"Paintkit" - before (L835)', { name, item });
                    ...
                    log('"Paintkit" - after (L856)', { name, item });
                    break;
                }
            }
        ...
        if (name.includes('(paint: ')) {
            log('"Painted" - before loop (L903)', { name, item });
            ...
            for (const paintC in this.paints) {
                ...
                if (name.includes(paint)) {
                    log('"Painted" - in loop, before (L910)', { name, item });
                    name = name.replace(paint, '').trim();
                    item.paint = this.paints[paintC];
                    log('"Painted" - after (L913)', { name, item });
                    break;
                }
            }
        }
        ...
        if (name.includes('kit fabricator') && item.killstreak > 1) {
            log('"Kit Fabricator" - before (L920)', { name, item });
            ...
            log('"Kit Fabricator" - after (L943)', { name, item });
        }
        ...
        if (
            ...
            log('"Collector\'s Chemistry Set" - before (L950)', { name, item });
            ...
            log('"Collector\'s Chemistry Set" - after (L969)', { name, item });
        }

        if (name.includes('strangifier chemistry set')) {
            log('"Strangifier Chemistry Set" - before (L973)', { name, item });

            name = name.replace('strangifier chemistry set', '').trim();
            const schemaItem = this.getItemByItemName(name);
            if (!schemaItem) {
                log('returned L978', { name, item });
                return item;
            }
            ...
            log('"Strangifier Chemistry Set" - after (L989)', { name, item });
        }

        if (name.includes('strangifier')) {
            log('"Strangifier" - before (L993)', { name, item });
            ...
            const schemaItem = this.getItemByItemName(name);
            if (!schemaItem) {
                log('returned L1000', { name, item });
                return item;
            }

            item.target = schemaItem.defindex;
            item.quality = item.quality ?? schemaItem.item_quality; //default quality
            log('"Strangifier" - after (L1006)', { name, item });
        }

        if (name.includes('kit') && item.killstreak) {
            log('"Kit" - before (L1010)', { name, item });
            ...
            log('"Kit" - after (L1034)', { name, item });
        }

        if (item.defindex) {
            log('returned L1038', { name, item });
            return item;
        }

        if (typeof item.paintkit === 'number' && name.includes('war paint')) {
            log('"War Paint" - before (L1043)', { name, item });
            ...
            if (name.includes('salvaged mann co. supply crate #')) {
                ...
                log('"Mann Co. Supply Crate" (L1076)', { name, item });
            ...
            if (schemaItem.item_class === 'supply_crate') {
                log('with "supply_crate" - before (L1139)', { name, item });
                item.crateseries = this.crateSeriesList[item.defindex];
                log('with "supply_crate" - after (L1141)', { name, item });
            } else if (schemaItem.item_class !== 'supply_crate' && number !== null) {
                log('not "supply_crate" and number !== null - before (L1143)', { name, item });
                item.craftnumber = number;
                log('not "supply_crate" and number !== null - after (L1145)', { name, item });
            }
        }

        log('Final returned L1149', { name, item });
        return item;
    }
    ...
    getItemByDefindex(defindex) {
        const index = this._itemsByDefindex.get(defindex);
        return typeof index === 'number' ? this.raw.schema.items[String(index)] : null;
    }
    ...
    getItemByItemName(name) {
        const index = this._itemsByName.get(removeThe(normalizeNameKey(name)));
        return typeof index === 'number' ? this.raw.schema.items[String(index)] : null;
    }
    ...
    getPaintKits() {
        const paintkits = this.raw.schema.paintkits;
        return Object.keys(paintkits).reduce((obj, id) => {
            obj[paintkits[id]] = +id;
            return obj;
        }, {});
    }
