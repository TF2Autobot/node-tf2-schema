...
class Schema {
    ...
    constructor(data) {
        ...
        this.craftableWeaponsSchema = null;
    }
    ...
    getCraftableWeaponsSchema() {
        if (this.craftableWeaponsSchema) {
            return this.craftableWeaponsSchema;
        }

        // Includes Jungle Inferno update weapons
        const weaponsToInclude = [1178, 1179, 1180, 1181, 1190];
        ...
        this.craftableWeaponsSchema =  this.raw.schema.items.filter(
            item =>
                weaponsToInclude.includes(item.defindex) ||
    ...
    getName(item, proper = true, usePipeForSkin) {
        ...
        if (item.quality2) {
            // Elevated quality
            name +=
                this.getQualityById(item.quality2) + (item.wear !== null || item.paintkit !== null ? '(e)' : '') + ' ';
        }
